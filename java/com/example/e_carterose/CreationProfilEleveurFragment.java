        ...
        buttonCreaCompte.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (databaseAccess.isElevageExists(numeroElevage)) {
                    ...
                    if (!databaseAccess.isNumElevageExists(numeroElevage)) {
                        Toast.makeText(getContext(), "Le numéro d'élevage entré n'existe pas", Toast.LENGTH_SHORT).show();
                        return;
                    }
                    long result = databaseAccess.createElevageProfile(numeroElevage, nomElevage, motDePasse, "1");

                    if (result != -1) {
                        Toast.makeText(getContext(), "Profil créé avec succès", Toast.LENGTH_SHORT).show();
                        // Revenir à la page de connexion
                        getActivity().getSupportFragmentManager().beginTransaction().replace(R.id.container, new ConnexionEleveurFragment()).commit();
                    } else {
                        Toast.makeText(getContext(), "Une erreur est survenue lors de la création du profil", Toast.LENGTH_SHORT).show();
                    }
                }
                // Réinitialiser les champs de saisie
                editTextNumeroElevage.setText("");
                editTextNomElevage.setText("");
